...
cdef class Abstract_Environment_Iterator:
    ...
    def get_results(self, model: Normal_model) -> float:
        ...
        cdef real_t[:, ::1] input_states = np.zeros((self.number_of_environments, model_cython.get_normal_input_size()), dtype = real_t_numpy)
        cdef real_t[:, ::1] outputs
    ...
    cdef int iterate_insert_state(self, input_state: real_t[:, ::1]) noexcept nogil:
        """
    ...
    cdef double iterate_react(self, real_t[:, ::1] output_values) noexcept nogil:
        """
